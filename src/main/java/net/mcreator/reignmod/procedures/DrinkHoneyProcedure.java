	...
	private static void execute(@Nullable Event event, Entity entity, ItemStack itemstack) {
		...
		if (itemstack.getItem() == ReignModModItems.HONEY_CONCENTRATE.get()) {
			if (entity instanceof LivingEntity _entity && !_entity.level().isClientSide())
				_entity.addEffect(new MobEffectInstance(ReignModModMobEffects.BEE_EFFECT.get(), 7200, 0, false, false));
			CheckBeeResistProcedure.execute(entity);
		} else if (itemstack.getItem() == Items.HONEY_BOTTLE) {
			if (entity instanceof LivingEntity _entity && !_entity.level().isClientSide())
				_entity.addEffect(new MobEffectInstance(ReignModModMobEffects.BEE_EFFECT.get(), 640, 0, false, false));
			CheckBeeResistProcedure.execute(entity);
		}
	}
}
