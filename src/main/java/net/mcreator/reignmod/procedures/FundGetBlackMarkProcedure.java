...
public class FundGetBlackMarkProcedure {
	public static void execute(LevelAccessor world, double x, double y, double z, Entity entity) {
		...
		if ((entity instanceof ItemEntity _itemEnt ? _itemEnt.getItem() : ItemStack.EMPTY).getItem() == ReignModModItems.ACCURSED_BLACK_MARK.get()) {
			KingdomData.setSourceDisturbance(KingdomData.getSourceDisturbance() + ((int) count * 10));
			...
			if (world instanceof ServerLevel _level)
				_level.sendParticles((SimpleParticleType) (ReignModModParticleTypes.EREOUS.get()), x, y, z, (int) (5 + KingdomData.getSourceDisturbance() / 100), 0.3, 1, 0, 0.5);
			...
			KingdomData.setSourceDisturbance(KingdomData.getSourceDisturbance() - ((int) count * 12));
			if (world instanceof Level _level) {
				if (!_level.isClientSide()) {
					_level.playSound(null, BlockPos.containing(x, y, z), ForgeRegistries.SOUND_EVENTS.getValue(new ResourceLocation("reign_mod:fall_source")), SoundSource.BLOCKS, 1, 1);
				} else {
			...
			if (world instanceof ServerLevel _level)
				_level.sendParticles((SimpleParticleType) (ReignModModParticleTypes.EREOUS.get()), x, y, z, (int) (KingdomData.getSourceDisturbance() / 100), 0.3, 1, 0, 0.5);
			if (!entity.level().isClientSide())
				entity.discard();
		}
	}
